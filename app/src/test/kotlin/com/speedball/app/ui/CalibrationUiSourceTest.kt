...
class CalibrationUiSourceTest {
    ...
    fun calibrationUiUsesDistanceInputAndDraggableVerticalCaliperLines() {
        ...
        assertTrue(app.contains("val showSetupOverlay = setupModeActive && !captureActive"))
        assertTrue(app.contains("if (showSetupOverlay) {"))
        assertTrue(app.contains("drawCircle(color = sampleColor"))
        assertTrue(app.contains("currentOnSetImpactZoneCornerFromPreview"))
        assertTrue(app.contains("currentOnSetBallBoxCornerFromPreview"))
        assertTrue(app.contains("OverlayButton(\"Impact\""))
        assertTrue(app.contains("OverlayButton(\"Ball Box\""))
        assertTrue(mainActivity.contains("normalizedOverlayPoint("))
        assertTrue(mainActivity.contains("x = (x / viewWidth.toFloat()).coerceIn(0f, 1f).toDouble()"))
        assertTrue(mainActivity.contains("y = (y / viewHeight.toFloat()).coerceIn(0f, 1f).toDouble()"))
        assertFalse(mainActivity.contains("normalizedPreviewDragPoint("))
        assertTrue(app.contains("val ballOffsets = ballBox.points.map"))
        assertTrue(app.contains("val impactOffsets = impactZone.points.map { it.toOffset(size) }"))
        assertTrue(app.contains("val ballOffsets = ballBox.points.map { it.toOffset(size) }"))
        assertTrue(app.contains("drawLine(color = Color(0xffff4f7b)"))
        assertTrue(mainActivity.contains("val points = current.points.toMutableList()"))
        assertTrue(mainActivity.contains("Phase14WorkflowEvent.ExpectedBallBoundsSelected(NormalizedFramePolygon(points))"))
        assertFalse(mainActivity.contains("val opposite = when (cornerIndex.coerceIn(0, 3))"))
        ...
        assertTrue(mainActivity.contains("VOICE_CLEAR_COMMANDS"))
        assertTrue(mainActivity.contains("isClearCommand(text: String)"))
        assertTrue(mainActivity.contains("VOICE_CLEAR_COMMAND recognized"))
        assertTrue(mainActivity.contains("handleClearCommand()"))
        assertTrue(mainActivity.contains("dismissVisualEstimateReport(report.attemptId)"))
        assertTrue(mainActivity.contains("REPORT_AUTO_CLEAR_DELAY_MILLIS = 10_000L"))
        assertTrue(mainActivity.contains("scheduleReportAutoClear(currentVisualEstimateReport)"))
        assertTrue(mainActivity.contains("REPORT_AUTO_CLEAR attempt="))
        assertTrue(mainActivity.contains("currentVisualEstimateReport = null"))
        assertTrue(mainActivity.contains("dismissedVisualEstimateAttemptId = visualEstimateAttemptId"))
        ...
        assertTrue(mainActivity.contains("VOICE_SHOOT_COMMAND partial ignored"))
        assertTrue(mainActivity.contains("VOICE_RECORD_COMMAND partial ignored"))
        val partialResultsPath = mainActivity.substringAfter("override fun onPartialResults")
            .substringBefore("override fun onEvent")
        assertFalse(partialResultsPath.contains("handleShootCommand()"))
        assertFalse(partialResultsPath.contains("stopVoiceRecordListener()"))
        val voiceShootCommandsPath = mainActivity.substringAfter("val VOICE_SHOOT_COMMANDS = setOf(")
            .substringBefore(")")
        assertTrue(voiceShootCommandsPath.contains("\"shoot\""))
        assertFalse(voiceShootCommandsPath.contains("\"shot\""))
        val voiceRecordCommandsPath = mainActivity.substringAfter("val VOICE_RECORD_COMMANDS = setOf(")
            .substringBefore(")")
        assertTrue(voiceRecordCommandsPath.contains("\"record\""))
        assertTrue(voiceRecordCommandsPath.contains("\"cheese\""))
        assertTrue(voiceRecordCommandsPath.contains("\"smile\""))
        assertFalse(voiceRecordCommandsPath.contains("\"recording\""))
        assertFalse(voiceRecordCommandsPath.contains("\"shutter\""))
        assertFalse(voiceRecordCommandsPath.contains("\"capture\""))
        assertFalse(voiceRecordCommandsPath.contains("\"start\""))
        assertFalse(voiceRecordCommandsPath.contains("\"go\""))
        ...
        val shootReadinessPath = mainActivity.substringAfter("private fun shootNotReadyStatus()")
            .substringBefore("private fun phase14ColorNotReadyStatus()")
        assertFalse(shootReadinessPath.contains("phase14ColorNotReadyStatus()"))
        assertFalse(shootReadinessPath.contains("return colorReadiness.message"))
        ...
        assertFalse(mainActivity.contains("Set the ball-flight ROI before measuring."))
        assertTrue(mainActivity.contains("buildImportColor(importGeometry, requireRegionOfInterest = false).readiness"))
        assertTrue(mainActivity.contains("Recorded estimate needs valid distance setup before processing."))
        assertTrue(app.contains("Cal plane ft"))
        assertTrue(app.contains("Ball plane ft"))
        assertTrue(app.contains("Shape ratio"))
        assertTrue(app.contains("Launch ft"))
        assertTrue(app.contains("KeyboardOptions(keyboardType = KeyboardType.Decimal)"))
        assertTrue(app.contains("cursorBrush = SolidColor(Color.White)"))
        assertTrue(mainActivity.contains("currentScaleModeOrNull()"))
        assertTrue(mainActivity.contains("VisualEstimateScaleMode.DepthCorrected("))
        assertTrue(mainActivity.contains("parseMotionBlobSideRatio(motionBlobSideRatioText)"))
        assertTrue(mainActivity.contains("maxCandidatePrincipalAxisRatio = motionBlobSideRatio"))
        assertTrue(mainActivity.contains("parseLaunchHeightFeet(launchHeightFeetText)"))
        assertTrue(mainActivity.contains("launchHeightFeet = launchHeightFeet"))
        assertTrue(mainActivity.contains("INITIAL_LAUNCH_HEIGHT_FEET_TEXT = \"4.0\""))
        assertTrue(mainActivity.contains("minEstimateMilesPerHour = null"))
        assertTrue(mainActivity.contains("minimumSpeedGatePolicy = MinimumSpeedGatePolicy.DISABLED"))
        val recordedWindowPath = mainActivity.substringAfter("private fun runRecordedWindowEstimate(")
            .substringBefore("private fun proofTraceLinesForRecordedEstimate(")
        assertFalse(recordedWindowPath.contains("ROI before processing."))
        assertFalse(recordedWindowPath.contains("physicalDetectorConfig = RecordedHfrPhysicalDetectorConfig()"))
        ...
        assertFalse(mainActivity.contains("playReadySignalThenRecordedHfrEstimate()"))
        assertTrue(mainActivity.contains("readySignalPending"))
        assertTrue(mainActivity.contains("startSoundTriggeredRecordingEstimate()"))
        assertTrue(mainActivity.contains("handleImpactAudioArmed(attemptId, mode, session)"))
        assertTrue(mainActivity.contains("startRecordedHfrAfterImpactAudioArmed(attemptId, mode, session)"))
        assertTrue(mainActivity.contains("handleRecordedHfrFirstFrameAnchor(attemptId, mode, session, anchor)"))
        assertTrue(mainActivity.contains("session.enableAcceptance(acceptAfterSampleIndex)"))
        assertTrue(mainActivity.contains("stopVoiceRecordListener()"))
        assertTrue(mainActivity.contains("stopMode = BurstStopMode.ExternalStop"))
        assertTrue(mainActivity.contains("@Volatile private var activeImpactMapping"))
        assertTrue(mainActivity.contains("@Volatile private var activeImpactAudioSession"))
        assertTrue(mainActivity.contains("@Volatile private var activeImpactVideoAnchor"))
        assertTrue(mainActivity.contains("RECORDED_HFR_AUDIO_ARMED"))
        assertTrue(mainActivity.contains("RECORDED_HFR_FIRST_FRAME_ANCHOR"))
        assertTrue(mainActivity.contains("RECORDED_HFR_MARKER_ACCEPTANCE_ENABLED"))
        assertTrue(mainActivity.contains("RECORDED_HFR_IMPACT_DETECTED"))
        assertTrue(mainActivity.contains("RECORDED_ESTIMATE_WINDOW"))
        ...
        assertTrue(mainActivity.contains("CandidateReductionBudget("))
        assertTrue(mainActivity.contains("RECORDED_HFR_MAX_BLOBS_PER_FRAME"))
        assertTrue(mainActivity.contains("RECORDED_HFR_MAX_TOTAL_CANDIDATE_BLOBS"))
        assertTrue(mainActivity.contains("RECORDED_HFR_MAX_RANSAC_PAIR_HYPOTHESES"))
        assertTrue(mainActivity.contains("proofOnly = earlyExposureNoRead != null"))
        assertTrue(mainActivity.contains("cancellationSignal = windowCancellation"))
        val liveShootPath = mainActivity.substringAfter("private fun startSoundTriggeredRecordingEstimate(")
        ...
        assertTrue(mainActivity.contains("uniqueSensorTs=\${outcome.diagnostics.uniqueTimestampCount}"))
        ...
        assertTrue(mainActivity.contains("READY_CUE_ACCEPT_MARGIN_MILLIS"))
        ...
        assertFalse(mainActivity.contains("updateShellState(status = \"Ready to shoot\")"))
        assertTrue(mainActivity.contains("updateShellState(status = \"Arming impact audio\")"))
        assertTrue(mainActivity.contains("Say shoot, record, cheese, or smile"))
        assertTrue(mainActivity.contains("Listening for shoot/record/cheese/smile"))
        ...
        val idleRecognizerPath = mainActivity.substringAfter("if (isIdleVoiceRecognizerError(error))")
            .substringBefore("voiceConsecutiveErrors += 1")
        assertTrue(idleRecognizerPath.contains("SpeedBallRunCommandState.Listening"))
        assertFalse(idleRecognizerPath.contains("SpeedBallRunCommandState.VoiceRetrying"))
        assertTrue(idleRecognizerPath.contains("VOICE_LISTENING_PROMPT"))
        val activeStatusPath = mainActivity.substringAfter("private fun isCaptureActiveStatus(): Boolean")
            .substringBefore("private fun startSoundTriggeredRecordingEstimate()")
        assertTrue(activeStatusPath.contains("captureStatus.equals(\"Recording\""))
        assertFalse(activeStatusPath.contains("contains(\"record\", ignoreCase = true)"))
        assertTrue(activeStatusPath.contains("window decoding"))
        val reportingPath = mainActivity.substringAfter("private fun resumeRunModeCommandPath()")
            .substringAfter("if (currentVisualEstimateReport?.kind == VisualEstimateReportKind.Success) {")
            .substringBefore("val notReadyStatus = shootNotReadyStatus()")
        assertTrue(reportingPath.contains("startVoiceRecordListener()"))
        ...
        assertTrue(mainActivity.contains("UtteranceProgressListener"))
        assertTrue(mainActivity.contains("TextToSpeech"))
