#!/bin/bash
# wait_for_codex.sh - optional caller-side blocker until Codex writes a response.
#
...
if [[ "$TIMEOUT" == "0" ]]; then
    echo "[wait] $TIMESTAMP - polling ${RESPONSE_FILE} for Codex response (no timeout)"
else
    echo "[wait] $TIMESTAMP - polling ${RESPONSE_FILE} for Codex response (timeout: ${TIMEOUT}s)"
fi
