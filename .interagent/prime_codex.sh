#!/bin/bash
# prime_codex.sh — Send a short bootstrap/re-prime message into the running
# Codex tmux session so it re-enters the doorbell protocol.
...
NEXT_JSON="$($BROKER next-ready-task)"
mapfile -t TRACKING_STATE < <(
    STATUS_JSON="$STATUS_JSON" NEXT_JSON="$NEXT_JSON" node <<'NODE'
const status = JSON.parse(process.env.STATUS_JSON);
const next = JSON.parse(process.env.NEXT_JSON);
const gate = status.current_task_gate || next.current_task_gate || { required: false, valid: true, workstream_id: null, task_kind: null, next_step: null, issues: [] };
...
if [[ "$TRACKING_REQUIRED" == "1" && "$TRACKING_VALID" == "1" && "$CURRENT_TASK_ID" != "" && "$CURRENT_IMPLEMENTER" == "codex" && "$CURRENT_ACTIVE_OWNER" == "codex" && "$CURRENT_AWAITING" == "codex" ]]; then
...
if [[ -n "$ACTIVE_REVIEW_TASK" ]]; then
    BOOTSTRAP+=" Claude review is still in flight for task: ${ACTIVE_REVIEW_TASK}. Treat that as a waiting lane, not an idle lane."
fi
if [[ -n "$NEXT_TASK" ]]; then
    BOOTSTRAP+=" Next ready task: ${NEXT_TASK}. This manual prime means a Claude review is already blocking another step and this queued task is the next ready parallel lane in the same workstream. Start it now unless the tracked workstream/write-scope rules block it."
...
echo "[sent] Re-primed Codex session '$SESSION' (reason: $REASON${NEXT_TASK:+, next task: $NEXT_TASK})"
